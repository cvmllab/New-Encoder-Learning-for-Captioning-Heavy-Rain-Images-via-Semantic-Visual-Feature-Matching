...
class AtJ(nn.Module):
    def __init__(self):
        ...
        self.decoder_s = Dense_decoder(out_channel=3) #rain
    ...
    def forward(self, x, activation = 'sig'):
        ...
        ## 8 X 8
        x4 = self.trans_block4(self.dense_block4(x3)) #encoder 

        ######################################decoder
        J = self.decoder_J(x, x1, x2, x4)
        A = self.decoder_A(x, x1, x2, x4)
        t = self.decoder_t(x, x1, x2, x4, activation='sig')
        s = self.decoder_s(x, x1, x2, x4, activation='sig') 

        t = torch.abs((t)) + (10 ** -10)
        t = t.repeat(1, 3, 1, 1)

        
        modeling_J = (x - (1 - t) * A) / (t + 0.0001) - s

        return  modeling_J, A, t, s
